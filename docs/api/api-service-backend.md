...
**POST** `/ai/chat/sessions` | Bearer Token 필요
...
**GET** `/ai/chat/sessions` | Bearer Token 필요
...
**GET** `/ai/chat/sessions/{sessionId}/messages` | Bearer Token 필요
...
| page | Integer | X    | 페이지 (default: 0)  |
| size | Integer | X    | 사이즈 (default: 20) |
| sort | String  | X    | 정렬 기준 (예: messageId,asc) |
...
**DELETE** `/ai/chat/sessions/{sessionId}` | Bearer Token 필요
...
| 상황           | 코드   | 메시지                              |
| -------------- | ------ | ----------------------------------- |
| 이미 종료 세션 | AI_004 | 이미 종료(CLOSED) 처리된 세션입니다 |

---

## 19-5. 메시지 저장 (AI 서버 내부 호출)

**POST** `/ai/chat/messages/record` | Bearer Token 필요

> AI 서버가 대화 처리 후 백엔드에 메시지를 저장하기 위해 내부적으로 호출하는 API.
> 프론트엔드 직접 호출 대상 아님.
...
| ---------- | ------ | ---- | ------------------------------------- |
| sessionId  | Long   | O    | 채팅 세션 ID                          |
| role       | String | O    | USER / AI                             |
| content    | String | O    | 메시지 내용                           |
| intent     | String | X    | ASSET / STOCK / TRANSFER / UNKNOWN    |
| actionType | String | X    | 액션 유형 (예: PIN_REQUIRED)          |
