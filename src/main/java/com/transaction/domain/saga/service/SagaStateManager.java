...
public class SagaStateManager {
    ...
    public SagaTransaction initSaga(SagaType type, String transactionKey, String outboxPayload) {
        ...
        outboxService.save("SAGA", saga.getSagaId(), "saga.started", KafkaTopics.SAGA_STARTED,
                String.valueOf(saga.getSagaId()), outboxPayload);
        return saga;
    }

    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void recordStep(Long sagaId, SagaStepName name, int order, SagaStepStatus status,
            String requestPayload, String responsePayload, String errorMessage) {
        sagaStepHistoryRepository.save(
                SagaStepHistory.create(sagaId, name, order, status, requestPayload, responsePayload, errorMessage));
    }

    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void completeSaga(Long sagaId, String idempotencyKey, String responsePayload, String outboxPayload) {
        SagaTransaction saga = findSaga(sagaId);
        saga.updateStatus(SagaStatus.SUCCESS);
        sagaTransactionRepository.save(saga);
        outboxService.save("SAGA", sagaId, "saga.completed", KafkaTopics.SAGA_COMPLETED,
                String.valueOf(sagaId), outboxPayload);
    ...
    public void failSaga(Long sagaId, String idempotencyKey, String reason, String outboxPayload) {
        ...
        sagaTransactionRepository.save(saga);
        outboxService.save("SAGA", sagaId, "saga.failed", KafkaTopics.SAGA_FAILED,
                String.valueOf(sagaId), outboxPayload);
    ...
    public void startCompensation(Long sagaId, String outboxPayload) {
        ...
        sagaTransactionRepository.save(saga);
        outboxService.save("SAGA", sagaId, "saga.compensated", KafkaTopics.SAGA_COMPENSATED,
                String.valueOf(sagaId), outboxPayload);
    }
    ...
    public void unknownSaga(Long sagaId, String idempotencyKey, String reason, String outboxPayload) {
        ...
        sagaTransactionRepository.save(saga);
        outboxService.save("SAGA", sagaId, "saga.unknown", "saga.unknown",
                String.valueOf(sagaId), outboxPayload);
        idempotencyService.fail(idempotencyKey);
    }

    private SagaTransaction findSaga(Long sagaId) {
        return sagaTransactionRepository.findById(sagaId)
